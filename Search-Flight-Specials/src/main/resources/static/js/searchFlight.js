...
originInput.addEventListener('input', async function() {
	const originValue = originInput.value; // 現在入力された値を取得
	
	// 入力値が空の場合、検索結果リストをクリアして終了
	if (!originValue) {
	    ...
	    try {
	        // /api/search-iataエンドポイントにGETリクエストを送信
	        const response = await fetch(`/api/search-iata?keyword=${originValue}`);

	        // レスポンスが成功したかどうかを確認
	        if (!response.ok) {
	            // レスポンスが404 Not Foundの場合、「一致する空港はありません」メッセージを表示
	            if (response.status === 404) {
	                originSearchResults.innerHTML = '<div class="dropdown-item disabled">入力値と一致する空港が存在しません。</div>';
	            } else {
	                // その他のエラーが発生した場合、エラーメッセージを表示
	                originSearchResults.innerHTML = '<div class="dropdown-item disabled">検索中にエラーが発生しました。</div>';
	        ...
	        // 検索結果リストを生成
	        originSearchResults.innerHTML = ''; // 既存のリストをクリア
	        if (data.length === 0) {
	            // 検索結果がない場合、「一致する空港はありません」メッセージを表示
	            originSearchResults.innerHTML = '<div class="dropdown-item disabled">入力値と一致する空港が存在しません。</div>';
	        } else {
	            // 検索結果に基づいてドロップダウンリストを生成
	            data.forEach(item => {
	                const div = document.createElement('div');
	                div.className = 'dropdown-item';
	                div.textContent = item.label;
	                div.addEventListener('click', function() {
	                    originInput.value = item.value; // 選択したIATAコードをoriginInputに設定
	                    originSearchResults.innerHTML = ''; // 検索結果リストをクリア
	                });
	                originSearchResults.appendChild(div);
	            });
	        }
	    } catch (error) {
